...
class GenericGroupSkillBelief():
    def __init__(self, sname, sid, uids, p_L_0, p_S, p_G, p_T, n, discussion_time, solve_time, usb) -> None:
        self.sname = sname
        self.sid = sid
        self.users = {}
        self.t = 1
        ...
        for uid in uids:
            user = usb(uid=uid, sname=sname, sid=sid, p_L_0=p_L_0, p_S=p_S, p_G=p_G, p_T=p_T, E_k=discussion_time + solve_time, n=n)
            self.users[uid] = user
    ...
    def step(self, o_t, c_ts):
        ...
        for uid in c_ts:
            if len(self.users) > 1:
                ...
                other_H = 0.5
            self.users[uid].step(o_t, c_ts[uid], other_H)
...
def plot_beliefs(beliefs):
    for sid in beliefs:
        ...
        plt.title('Observation SID: ' + str(sid) + ' ' + belief.sname)
        ...
        for uid in belief.users:
            ...
            plt.title('Belief SID: ' + str(sid) + ' ' + belief.sname + ' UID: ' + str(uid))
