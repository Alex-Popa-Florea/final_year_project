...
from group_belief import GroupSkillBelief, plot_beliefs
from various_methods import CMCCTD_BKT

# Storing the believed masteries for all skills involved in a task
class FullMasteries():
...
def create_beliefs(task, uids, full_masteries, usb):
    beliefs = {}
...
# Initialising the cv for the user study
def initialise_cv():
    source = 1
...
# Closes the cv for the user study
def finish_cv(cap):
    cap.release()
    cv2.destroyAllWindows()

# Runs the algorithms for a task from a live video source
def run_task(user_masteries, uids, unique_uids, source, cap, model, detector, matrix_to_recolor, peg_size, frame_tilt, frame_circle, angle, board_size, store=True, show=True):
    ...
    beliefs = create_beliefs(task=task, uids=uids, full_masteries=user_masteries, usb=CMCCTD_BKT)
